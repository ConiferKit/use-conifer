#!/usr/bin/env bash
# publish-0.1.2.sh — finish the 0.1.2 release. RUN THIS IN YOUR OWN TERMINAL.
#
...
#
# WHAT 0.1.2 SHIPS: ConiferCapabilityError (TS + Python) — the typed,
# model-switchable 400 the gateway serves an image turn on a no-vision model
# (code: unsupported_parameter, param: messages), the new `error.param` field,
# and a chat() fallback chain that advances on it. The client half of the
# 2026-08-29 OpenTag incident fix; the gateway half is typhoon 1d167bc0.
#
#   ./scripts/publish-0.1.2.sh              # gate + build, stops before publish
#   ./scripts/publish-0.1.2.sh --publish    # gate + build + publish both
#
...
Artifacts are in ./dist-release. To publish for real:
    ...
    ./scripts/publish-0.1.2.sh --publish
